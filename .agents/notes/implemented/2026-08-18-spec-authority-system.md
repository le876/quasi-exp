...
Governance structure, scientific contract, and Git persistence are separate claims. Registry stores definitions and upstream relationships, while current-state owns the current experiment, attempt and locators. Each historical definition is interpreted and checked inside its declared scientific source commit; the current checkout does not preserve old binding files or bytes. Release map owns only scientific/public SHA mappings and the unpublished fixed-point set, not experiment membership. None of these structures implies scientific pass or caches result conclusions. GPT-5 Pro question/answer pairs are advisory decision input; user-adopted scientific rules move into protocol before they govern execution.
...
New sessions can load only the material relevant to their task. Repeated attempts reuse a definition without turning runtime history into catalog entries, and old definitions remain auditable without forcing historical implementation into the working tree. Config or explicit runner parameters own executable values, artifacts own results, and consultation records preserve external interpretation without displacing either owner. A direct runner needs no artificial launcher. Structural checks stay lightweight, while runner semantics require separate scientific-contract review and focused tests.
...
scripts/spec/verify_spec_system.py checks source-bound catalog integrity and the authority structure while exposing separate scientific-contract and persistence axes. tests/test_spec_system.py covers registry, pointers, release mapping, advisory budgets, Skills, Agent Notes, archive sealing and Git persistence behavior. Historical source gaps in the current checkout remain visible through --strict-history.

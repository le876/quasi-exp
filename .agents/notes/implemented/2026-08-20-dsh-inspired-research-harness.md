...
Use task-conditioned navigation, an explicit manifest of Unicode-character ceilings for accretion-prone standing docs, two routine repo Skills plus one archive-only Agent Notes Skill, registry schema v3, pointer-only current-state, and separate governance/scientific/persistence result axes. The registry catalogs static executable definitions; current-state owns the current experiment, attempt and locators. Ordinary Note authoring is owned by standing instructions and `.agents/notes/README.md`; only semantic deduplication, supersession and frozen archive mechanics warrant a dedicated Skill. Agent Note status is stored in a DSH-style header, the filename records the first-proposed date, and Git records later history. Skill UI metadata is optional; when present it is validated without forcing every Skill to carry product metadata.
...
- Keep a generic Agent Note authoring Skill: rejected because it repeats the root trigger and Note format owner without adding a distinct mechanism.
...
- Apply architecture simplification to scientific code now: deferred; this change only builds the harness and performs a read-only indirection audit.

## Consequences

Standing context stays small, while specialized workflows load only when needed. Durable behavior and process changes remain explainable without creating one Note per edit. Existing owners absorb only factual movement relevant to their decision, and repositories with no durable implemented decision need no placeholder Note. Archive sealing adds a small deterministic manifest only when an archive first exists; old bodies are frozen rather than reformatted. Scientific readiness remains outside governance verification.

Reconsider this split if task-conditioned routing consistently misses required evidence, Note updates become noisy, archive sealing obstructs legitimate recovery, or a real new workflow requires an additional Skill. Budget listing is advisory; hard ceilings apply only to the explicit standing-doc manifest, and a raise needs a local explanation rather than a new Note unless the policy itself changes.

## Verification

The repo-local Skills are checked with `quick_validate.py`. The verifier's default governance mode, report-only budget mode, Python compilation, `tests/test_spec_system.py`, and `git diff --check` are the direct implementation checks. Tests cover lifecycle formats, zero active implemented Notes, active-only present-state structure, frozen archives, optional Skill metadata, links, explicit budgets and Git persistence behavior.

...
Use task-conditioned navigation, advisory Unicode-character ceilings for accretion-prone standing docs, two routine repo Skills plus one archive-only Agent Notes Skill, a static experiment catalog, pointer-only current-state, and separate governance/scientific/persistence result axes. Historical catalog bindings are checked in their own scientific source commits rather than kept byte-identical in the current checkout. Release mapping records fixed-point publication status without repeating experiment membership. Ordinary Note authoring is owned by standing instructions and `.agents/notes/README.md`; only semantic deduplication, supersession and frozen archive mechanics warrant a dedicated Skill. Agent Note status is stored in a DSH-style header, the filename records the first-proposed date, and Git records later history. Skill UI metadata is optional; when present it is validated without forcing every Skill to carry product metadata.

...
| 可执行 experiment definition/binding | [registry.yaml](../spec/registry.yaml) | current state、README。 |
...
| scientific/public SHA 对应 | [release-map.yaml](../spec/release-map.yaml) | current state、release note。 |
...
- **experiment definition**：registry 中 source/protocol/config/runner/optional launcher/tests/upstream 的静态对应；不等于当前选择、正在运行、通过或授权。
- **scientific source fixed point**：某次科学实现绑定的 Git commit，不等于 public release SHA。
...
- **governance structure**：owner、路径、schema、预算、Skill、Note 与 seal 的结构一致性。
...
先读 [根 AGENTS.md](../AGENTS.md)，再按任务读取本页和 [registry](../spec/registry.yaml)。代码问题读 Architecture 与源码/tests；科学问题读绑定的 contract/config/runner；结果问题直接读 artifact 并核对 seal；当前定位才读 current state。历史材料保留 provenance-bound 路径。不要从版本名、commit message、binding 或 verifier 绿灯推断 scientific pass。

## Writing route

修改 `docs/` 前读 [docs/AGENTS.md](AGENTS.md)。Formal method、阈值、stage order 或 claim boundary 变化先建立 protocol revision，再同步 executable binding；不得根据已见结果回写合同。结果由 artifact 产生，seal 单独说明，current state 只更新 locator。

新增/修改本 authority system 的非写入式结构检查见 [testing.md](testing.md)。`--strict-history` 与默认 closure 是不同审计范围；历史缺口要报告，不能凭空补造来源。

...
SOURCE_FIXED_POINT_PATH = (
    SOURCE_ROOT
    / "docs/provenance/v12_12_retry3/source_fixed_point.json"
)
FAMILY_DISPOSITION_PATH = (
    SOURCE_ROOT
    / "docs/provenance/v12_12_retry3/family_disposition.json"
)
...
def stage_protocol(
    ...
    *,
    python: Path,
    source_state: SourceLockState | None = None,
) -> dict[str, Any]:
    locked_source = source_state or preflight_source_fixed_point(
        SOURCE_ROOT,
        SOURCE_FIXED_POINT_PATH,
        requested_config=Path(str(config["config_path"])),
    )
    if output_root.exists() and (
        not output_root.is_dir() or any(output_root.iterdir())
    ):
        raise SourceLockError(
            "protocol stage requires a new, empty output root"
        )
    family_disposition = load_family_disposition(FAMILY_DISPOSITION_PATH)
    disposition_entries = validate_family_disposition(family_disposition)
    write_protocol_source_snapshot(locked_source, output_root)
    ...
    development_pairs = {
        (str(item["family_id"]), str(item["family_fingerprint"]))
        for item in disposition_entries
    }
    development_ids = {family_id for family_id, _ in development_pairs}
    development_fingerprints = {
        fingerprint for _, fingerprint in development_pairs
    }
    stress_pairs = {
        (str(row["family_id"]), str(row["family_fingerprint"]))
        for row in catalog.loc[
            catalog["group_id"].eq(STRESS_GROUP),
            ["family_id", "family_fingerprint"],
        ].to_dict("records")
    }
    core_rows = catalog.loc[
        catalog["group_id"].eq(CORE_GROUP),
        ["family_id", "family_fingerprint"],
    ].to_dict("records")
    ...
    implementation = {
        ...
        "source_fixed_point": locked_source.manifest_sha256,
        "family_disposition": sha256_file(FAMILY_DISPOSITION_PATH),
    }
    atomic_write_json(
        stage / "source_manifest.json",
        {
            ...
            "source_lock": {
                "git_sha": locked_source.git_sha,
                "source_fixed_point_path": (
                    locked_source.manifest_relative_path
                ),
                "source_fixed_point_sha256": (
                    locked_source.manifest_sha256
                ),
                "protocol_source_snapshot": (
                    "00_protocol/source_snapshot/source_lock.json"
                ),
            },
    ...
    checks = {
        ...
        "stress_families_are_development_boundary_families": (
            len(stress_pairs) == 4 and stress_pairs == development_pairs
        ),
        "core_families_remain_historical_formal_core_validation": all(
            str(row["family_id"]) not in development_ids
            and str(row["family_fingerprint"])
            not in development_fingerprints
...
def evaluation_worker(args: argparse.Namespace) -> int:
    ...
    for family_id, family_reference in reference.groupby(
        ...
        assignment_report: dict[str, Any] = {
            "success": False,
            "reason": "teacher_gold_reference_unavailable",
        }
        if teacher_gate_pass:
        ...
        if assignment_succeeded:
            ...
            metrics = {
                ...
                "validation_beta_rms_p50_deg": None,
                "validation_beta_rms_p95_deg": None,
                "validation_beta_rms_max_deg": None,
                "validation_beta_abs_p50_by_joint_deg": unavailable,
                "validation_beta_abs_p95_by_joint_deg": unavailable,
                "validation_beta_abs_max_by_joint_deg": unavailable,
                "validation_beta_signed_bias_by_joint_deg": unavailable,
                "validation_rowspace_beta_rms_p50_deg": None,
                "validation_rowspace_beta_rms_p95_deg": None,
                "validation_nullspace_beta_rms_p50_deg": None,
                "validation_nullspace_beta_rms_p95_deg": None,
                "validation_null_energy_fraction_p50": None,
                "validation_null_energy_fraction_p95": None,
                "validation_jacobian_sigma3_p05_m": None,
                "validation_fk_p50_mm": float(
                    np.percentile(fk_error_mm, 50)
                ),
                "validation_fk_p95_mm": float(
                    np.percentile(fk_error_mm, 95)
                ),
                "validation_fk_max_mm": float(np.max(fk_error_mm)),
                "prediction_near_saturation_fraction": None,
                "predicted_minimum_joint_margin_deg": float(
                    np.min(point_margin)
                ),
                "predicted_margin_lt_1p5_fraction": float(
                    np.mean(point_margin < 1.5)
                ),
            }
        joint_p95 = list(
            np.nan if value is None else float(value)
            for value in metrics["validation_beta_abs_p95_by_joint_deg"]
        )
        checks = {
        ...
        gold_rms = (
            np.rad2deg(
                np.sqrt(
                    np.mean(
                        np.square(prediction - assigned_beta), axis=1
                    )
                )
            )
            if assignment_succeeded
            else np.full(len(prediction), np.nan, dtype=float)
        )
        report = {
            "seed": seed,
            ...
            "worst_joint_abs_p95_deg": (
                max(joint_p95) if assignment_succeeded else None
            ),
            **metrics,
            **trajectory,
        }
    ...
    atomic_write_json(
        destination / "report.json",
        {
            "seed": seed,
            "families": reports,
            "all_teacher_valid_assignments_succeeded": all(
                (
                    not family["checks"][
                        "teacher_independent_hard_cycle_available"
                    ]
                )
                or family["gold_assignment"]["success"]
    ...
    *,
    ...
    return _gate(
        stage / "gate.json",
        {
            ...
            "all_core_prediction_to_gold_assignments_succeeded": bool(
                metrics.loc[
                    metrics["group_id"].eq(CORE_GROUP),
                    "gold_assignment_success",
                ].all()
            ),
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    requested_config = Path(args.config).resolve()
    source_state = preflight_source_fixed_point(
        SOURCE_ROOT,
        SOURCE_FIXED_POINT_PATH,
    ...
    for name in requested:
        ...
        if name == "protocol":
            if gate_path.is_file():
                verify_protocol_source_snapshot(source_state, output_root)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=str(
            ...
            / (
                "configs/"
                "bacra_v12_12_1_multiresolution_teacher_retry3.yaml"
            )
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    verify_protocol_source_snapshot(
        source_state, Path(report["output_root"])
    )

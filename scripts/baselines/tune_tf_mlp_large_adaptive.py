...
class AdaptiveController:
    def __init__(self) -> None:
        ...
        self.alpha_grid = [1e-7, 3e-7, 1e-6]
        self.lr_grid = [8e-4, 1e-3, 1.2e-3]
        self.batch_grid = [512, 1024, 2048]
        self.epochs_grid = [800, 1200, 1800]
        self.patience_grid = [60, 90, 120]
        self.tol_value = 1e-5
        self.lambda_phys_grid = [0.0, 0.02, 0.05, 0.1]
    ...
    def _clip(self, knob: str, idx: int) -> int:
        bound = {
            ...
            "epochs": len(self.epochs_grid),
            "patience": len(self.patience_grid),
            "lambda_phys": len(self.lambda_phys_grid),
        }[knob]
        return min(max(idx, 0), bound - 1)
    ...
    def _regime(self, m: TrialMetrics) -> str:
        theta_gap = m.val_theta_mae_deg - m.train_theta_mae_deg
        tension_gap = m.val_tension_mae_n - m.train_tension_mae_n
        if theta_gap > 0.5 or tension_gap > 20.0:
            return "overfit"
        if theta_gap < 0.25 and tension_gap < 10.0:
            return "underfit"
        return "balanced"
    ...
    def propose(
        ...
        if previous_metrics is None:
            warmup = ("hidden", +1)
            candidate = self._apply_move(current_indices, warmup)
            return candidate, "warmup", warmup
        ...
        for knob in ["hidden", "epochs", "alpha", "lr", "batch", "patience", "lambda_phys"]:
            for delta in (-1, +1):
    ...
    def params_from_indices(self, indices: dict[str, int]) -> dict[str, Any]:
        return {
            "tf_mlp_large": {
                ...
                "max_iter": int(self.epochs_grid[indices["epochs"]]),
                "n_iter_no_change": int(self.patience_grid[indices["patience"]]),
                "tol": float(self.tol_value),
                "loss_lambda_phys": float(self.lambda_phys_grid[indices["lambda_phys"]]),
            }
        }
...
def _run_trial(
    ...
    metrics_path = out_dir / "tf_mlp_large" / "metrics.json"
...
def _summary_markdown(dataset: Path, split_file: Path, history: list[dict[str, Any]], best_trial: dict[str, Any], baseline_trial: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# TF-MLP-Large 20-Round Adaptive Tuning Summary")
    lines.append("")
...
def main() -> None:
    ...
    for round_number in range(1, int(args.rounds) + 1):
        ...
        print(f"\n=== ROUND {round_number:02d}/{args.rounds} | regime={regime} | move={move[0]}:{move[1]} ===", flush=True)
        ...
        row = {
            ...
            "params": params_payload["tf_mlp_large"],
    ...
    summary = _summary_markdown(args.dataset, args.split_file, history, best_row, baseline)
    (args.out_root / "SUMMARY.md").write_text(summary, encoding="utf-8")
    _plot_curves(history, args.out_root / "trial_curve.png")
    _write_yaml(args.out_root / "best_params.yaml", {"tf_mlp_large": best_row["params"]})

...
RETRY6_RUNNER="$SOURCE_ROOT/scripts/analysis/run_bacra_v14_2r_stitched_atlas.py"
RETRY6_CONFIG="$SOURCE_ROOT/configs/bacra_v14_2r_stitched_atlas_retry6.yaml"
RETRY6_OUTPUT="$PROJECT_ROOT/runs/bacra_v14_2r_stitched_atlas_retry6"
V143_PIPELINE="$SOURCE_ROOT/scripts/pipelines/run_bacra_v14_3_repaired_5k_student_v2.sh"
...
export MPLCONFIGDIR=/tmp/mpl-bacra-v14-2r-retry6

run_stage() {
  local stage_name=$1
  echo "stage=$stage_name status=starting_or_validated_resume"
  "$PYTHON_BIN" "$RETRY6_RUNNER" \
    --config "$RETRY6_CONFIG" \
    --output-root "$RETRY6_OUTPUT" \
    --stage "$stage_name"
...
if not bool(summary.get("deployment_authorized", False)):
    raise SystemExit("retry6 did not authorize repaired 5k; V14.3 remains blocked")
PY
